...
_BUFFER-NAME is the name for the terminal buffer (unused in vterm).
PROGRAM is the program to run.
SWITCHES are command line arguments for the program."
...
(defun claude-code--vterm-synchronize-scroll ()
  "Synchronize scrolling for vterm to prevent flickering.

This custom function keeps the prompt at the bottom of the window when
possible, preventing scrolling issues when switching buffers or when
the minibuffer opens/closes."
  (let ((windows (get-buffer-window-list (current-buffer) nil t)))
    (dolist (window windows)
      ;; Only adjust scrolling when not in copy mode
      (unless (and (boundp 'vterm-copy-mode) vterm-copy-mode)
        (with-selected-window window
          (let ((point-max (point-max)))
            ;; Check if we're near the end of the buffer
            (when (>= (point) (- point-max 2))
              ;; Keep prompt at bottom of window
              (goto-char point-max)
              (recenter -1))
            ;; Otherwise ensure point is visible
            (unless (pos-visible-in-window-p (point) window)
              (recenter))))))))

(defun claude-code--vterm-adjust-process-window-size-advice (orig-fun &rest args)
  ...
  (let* ((windows (get-buffer-window-list (current-buffer) nil t))
    ...
    (dolist (window windows)
      (let* ((window-width (window-body-width window))
             (last-width (gethash window claude-code--window-widths)))
        (when (or (not last-width) (/= window-width last-width))
          (setq width-changed t)
          (puthash window window-width claude-code--window-widths))))
    ;; Only call original function if width changed
    (when width-changed
      (apply orig-fun args))))
  ...
  (dolist (claude-buffer (claude-code--find-all-claude-buffers))
    (with-current-buffer claude-buffer
      ...
      (when-let ((windows (get-buffer-window-list claude-buffer nil t)))
        ;; Dispatch to appropriate backend's synchronize function
        (pcase claude-code-terminal-backend
          ('eat (claude-code--synchronize-scroll windows))
          ('vterm (claude-code--vterm-synchronize-scroll)))))))
